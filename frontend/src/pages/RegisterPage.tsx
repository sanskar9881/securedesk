...
const ROLES = [
  { role:"user" as Role,    label:"Employee",  icon:"👤", color:"teal",
    desc:"Scan files, view own history, use AI chat and phishing detector" },
  { role:"manager" as Role, label:"Manager",   icon:"👔", color:"purple",
    desc:"Full employee access + Activity logs, AI Copilot, UEBA, WhatsApp DLP" },
  { role:"admin" as Role,   label:"Admin",     icon:"🏢", color:"amber",
    desc:"Complete access — users, compliance, organization, billing, all features" },
];

const BORDER: Record<string,string> = {
  teal:   "border-teal-600/60 bg-teal-950/20",
  purple: "border-purple-600/60 bg-purple-950/20",
  amber:  "border-amber-600/60 bg-amber-950/20",
};
...
const BADGE: Record<string,string> = {
  teal:   "bg-teal-950/40 text-teal-400 border-teal-700/40",
  purple: "bg-purple-950/40 text-purple-400 border-purple-700/40",
...
export default function RegisterPage() {
  ...
  const doRegister = async () => {
    ...
    if (!name || name.length < 2)  { setErr("Enter your full name (at least 2 characters)"); return; }
    if (!id)                        { setErr("Enter your email address or phone number"); return; }
    if (!id.includes("@") && !/^\+?[\d\s\-]{8,}$/.test(id)) {
      setErr("Enter a valid email (you@gmail.com) or phone number (9876543210)"); return;
    }
    if (pw.length < 6)              { setErr("Password must be at least 6 characters"); return; }

    setLoad(true);
    try {
      const { data } = await api.post("/auth/register", { name, identifier: id, password: pw, role });
      // ✅ Store role correctly from API response
      ...
      navigate(r === "admin" || r === "manager" ? "/admin" : "/dashboard");
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">SecureDesk — AI Data Protection Platform</p>
        </div>

        {/* Step indicator */}
        <div className="flex items-center justify-center gap-3 mb-6">
          {[{n:1,label:"Choose Role"},{n:2,label:"Your Details"}].map((s,i) => (
            <div key={s.n} className="flex items-center gap-2">
              ...
              <span className={`text-sm ${step >= s.n ? "text-white":"text-gray-600"}`}>{s.label}</span>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-2xl">
          ...
          {step === 1 && (
            <div>
              ...
              <p className="text-gray-500 text-sm mb-5">This determines what you can see and do in SecureDesk</p>
              <div className="space-y-3 mb-6">
                {ROLES.map(opt => (
                  <button key={opt.role} onClick={() => setRole(opt.role)}
                    className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 transition-all text-left
                      ${role===opt.role ? BORDER[opt.color] : "border-gray-800 hover:border-gray-700"}`}>
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl flex-shrink-0
                      ${role===opt.role ? ICON_BG[opt.color] : "bg-gray-800"}`}>
                      {opt.icon}
                    </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-5">
                <button onClick={() => { setStep(1); setErr(""); }}
                  className="text-gray-500 hover:text-white text-sm transition">← Back</button>
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border ${BADGE[sel.color]}`}>
                  {sel.icon} {sel.label}
                </div>
              </div>

              <h2 className="text-white font-semibold text-lg mb-1">Your Details</h2>
              <p className="text-gray-500 text-sm mb-5">Fill in your information to create your account</p>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Email or Phone Number</label>
                  <div className="relative">
                    <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-600"/>
                    <input value={form.identifier}
                      onChange={e => { setForm({...form,identifier:e.target.value}); setErr(""); }}
                      placeholder="you@company.com  or  9876543210"
                      className="w-full bg-gray-800 border border-gray-700 rounded-xl pl-10 pr-4 py-3 text-white text-sm placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition"/>
                  </div>
                  <p className="text-gray-600 text-xs mt-1.5">Gmail, company email, or 10-digit Indian mobile number</p>
                </div>
                ...
                <button onClick={doRegister} disabled={load}
                  ...
                  {load ? "Creating Account..." : `Create ${sel.label} Account`}
                </button>
              </div>
            </div>
          )}
