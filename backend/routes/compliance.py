...
GET  /api/compliance/report/pdf    → PDF download
...
async def compliance_report(period: int = 30, user=Depends(get_current_user)):
    org_name = user.get("org_name", "Your Organization")
    data     = await gather_compliance_data(org_name, period)
...
async def compliance_score(user=Depends(get_current_user)):
    org_name = user.get("org_name", "Your Organization")
    data     = await gather_compliance_data(org_name, 30)
    return {
...
async def compliance_data(user=Depends(get_current_user)):
    org_name = user.get("org_name", "Your Organization")
    return await gather_compliance_data(org_name, 30)
